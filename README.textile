...
* Shifting a selection from a CSS (or LESS or SASS) file, sorts all attributes inside their selectors (alphabetically, vendor-attributes and vendor-styles at the end)</li>
* Shifting selected attribute-style lines inside a CSS (or LESS or SASS) file, sorts them (alphabetically, vendor-attributes and vendor-styles at the end)</li>
...
* Selected JsDoc block: Convert "@return" into "@returns", add curly brackets around data types in "@param" and "@returns" lines, correct invalid data types into existing primitive data types (bool => boolean, event => Object, int(eger) => number), reduce empty JsDoc lines
...
h2. Third Party Acknowledgment

Shifter uses the following third party components:

* "NaturalOrderComparator":https://github.com/paour/natorder/ by Pierre-Luc Paour
